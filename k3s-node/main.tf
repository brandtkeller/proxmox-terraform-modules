...
resource "proxmox_vm_qemu" "k3s_node" {
  ...
  provisioner "file" {
    ...
    destination = "/home/dev/install.sh"
    connection {
      type        = "ssh"
      user        = "dev"
      private_key = file("${var.ssh_priv_key_path}")
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo mv /home/dev/k3s-artifacts/k3s-${var.k3s_arch} /usr/local/bin/k3s",
      "sudo chmod +x /usr/local/bin/k3s",
      "sudo mkdir -p /var/lib/rancher/k3s/agent/images/",
      "sudo cp /home/dev/k3s-artifacts/k3s-airgap-images-${var.k3s_arch}.tar.zst /var/lib/rancher/k3s/agent/images/",
      "sudo mkdir -p /etc/rancher/k3s/",
      "sudo cp /home/dev/config.yaml /etc/rancher/k3s/config.yaml",
      "sudo INSTALL_K3S_SKIP_DOWNLOAD=true %{if var.role == "server"}INSTALL_K3S_EXEC='server'%{else}INSTALL_K3S_EXEC='agent'%{endif} sh /home/dev/install.sh",
    ]
    # Allow failure 
    on_failure = continue
